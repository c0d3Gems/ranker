<script context="module">
	export async function load({ page, fetch }) {
		...
		const result = await fetch(`/api/colegii/${idx}`, options);
		const countRequest = await fetch('/api/colegii/count');
...
<main>
	...
	<div class="content">
		...
		<Pagination {count} ipp={30} baseUrl={'/colegii/p/'} />
	</div>
</main>
<Footer />
